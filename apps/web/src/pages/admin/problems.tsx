...
export default function AdminProblemsPage() {
  ...
  const create = useMutation({
    mutationFn: (body: any) => api.post('/problems', body),
    onSuccess: () => {
  ...
  const update = useMutation({
    mutationFn: ({ id, ...body }: any) => api.patch(`/problems/${id}`, body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin-problems'] });
      setEditing(null);
    },
  ...
  return (
    <AdminLayout>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-xl font-semibold text-slate-900">Problems</h2>
      ...
      {!isLoading && (!data || data.length === 0) && (
        <EmptyState title="No problems" message="Create your first problem record." />
      )}
      {data?.map((p: any) => (
            ...
            <div className="min-w-0 flex-1">
              ...
              {p.assignedTo && (
                <p className="mt-1 text-xs text-slate-400">
                  Assignee: {p.assignedTo.firstName} {p.assignedTo.lastName}
                </p>
              )}
              {p.tickets?.length > 0 && (
                <p className="text-xs text-slate-400">{p.tickets.length} ticket(s) linked</p>
              )}
            </div>
      ...
      <ConfirmDialog
        ...
        description="This will permanently delete this problem record and its linked relations."
        confirmLabel="Delete"
      />
    </AdminLayout>
  );
